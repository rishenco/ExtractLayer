...
declares, and the service inserts the row, claims unleased rows and runs them. Between
processes, the table is the only channel.

**Claiming.** A worker claims a row by writing a fresh `claim_id` and `claimed_at`
(`FOR UPDATE SKIP LOCKED`). A row is claimable while its status is `running` and its
`claimed_at` is null or older than the TTL, so the lease alone separates a job being worked
from one waiting for a worker, and a job whose worker died comes back to the queue with no
...
the checkpoint, stay resumable: resuming returns the status to `running` and clears the
claim, keeps the checkpoint, and a claimer picks the row up again. `kill` ends the job for good. A claimer
