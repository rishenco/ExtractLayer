...
Verdict: SUPPORTED
Ran `grep -q 'Skips-plan-gate' .claude/agents/adversarial-reviewer.md && [ "$(grep -c '^   - Done:' work/merge-spec-into-plan/plan.md)" -eq 7 ] && [ "$(grep -c '^  - Correction from the build:' work/merge-spec-into-plan/plan.md)" -eq 1 ]` -> exit 0: `.claude/agents/adversarial-reviewer.md:10` is the phrase's only occurrence and reads "A change shipped under a `Skips-plan-gate:` trailer has no plan; judge those against `AGENTS.md` alone and say so."; the seven `- Done:` lines at `work/merge-spec-into-plan/plan.md:44,46,48,50,52,54,56` sit one under each of the seven steps at `:43,45,47,49,51,53,55`; the single `- Correction from the build:` at `:33` sits under the research bullet at `:32`, which claims `work/` has no slug directories and which the correction retracts; deleting the reviewer sentence, one `- Done:` line, or the correction line each makes the command exit 1.
