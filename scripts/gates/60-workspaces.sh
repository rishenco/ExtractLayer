...
while IFS= read -r manifest; do
  ...
  case "$(basename "$manifest")" in
    package.json)
      ...
      el_run_in "$dir" npm run --silent check || fail "$dir: npm run check failed"
      ;;
    go.mod)
      ...
      need golangci-lint "$dir" && { el_run_in "$dir" golangci-lint run ./... || fail "$dir: golangci-lint failed"; }
      el_run_in "$dir" go vet ./... || fail "$dir: go vet failed"
      el_run_in "$dir" go test ./... || fail "$dir: go test failed"
      ;;
    pyproject.toml)
      ...
      need ruff "$dir" && { el_run_in "$dir" ruff check . || fail "$dir: ruff failed"; }
      need mypy "$dir" && { el_run_in "$dir" mypy . || fail "$dir: mypy failed"; }
      need pytest "$dir" && { el_run_in "$dir" pytest -q || fail "$dir: pytest failed"; }
      ;;
  esac
done <<<"$manifests"
