...
- Delegate codebase search to the `codebase-researcher` subagent so its output never enters
  this context raw.
- After any change, run the `claim-auditor` subagent on what you assert, then the
  `adversarial-reviewer` subagent on the code. They are different jobs; do not merge them.
